...
void main(int argc, char** argv)
{
	...
	string str_temp
		...
		, fixing_holiday
		, fixing_lag
		, couponlegindex1_tenor
		, couponlegindex1_type
		, couponlegindex1fixed_freq
		, couponlegindex1floating_freq
		, couponlegindex1_holiday
		, couponlegindex2_tenor
		, couponlegindex2_type
		, couponlegindex2fixed_freq
		, couponlegindex2floating_freq
		, couponlegindex2_holiday;

	string couponlegrngchkindex_tenor
		, couponlegrngchkindex_type
		, couponlegrngchkindexfixed_freq
		, couponlegrngchkindexfloating_freq
		, couponlegrngchkindex_holiday
		, couponlegrngchkindex2_tenor
		, couponlegrngchkindex2_type
		, couponlegrngchkindex2fixed_freq
		, couponlegrngchkindex2floating_freq
		, couponlegrngchkindex2_holiday
		, call_freq
		, call_conv
		, call_stub
		, call_direction
	...
	double notional
		...
		, couponlegindex1_multi
		, couponlegindex2_multi
		, couponlegrngchkindex_multi
		, couponlegrngchkindex2_multi
		, caprate
		, floorrate
		, rahighbdry
		, ralowbdry
	...
	int int_tmp
		...
		, couponlegindx1_tenor
		, couponlegindex1fixed_frq
		, couponlegindx2_tenor
		, couponlegindex2fixed_frq
		, couponlegrngchkindx_tenor
		, couponlegrngchkindexfixed_frq
		, couponlegrngchkindx2_tenor
		, couponlegrngchkindex2fixed_frq
		, fix_lag
		, rahighbdryoh_flag
		, ralowbdryoh_flag
		, nshift
		...
		, call_frq
		, callperiod
		, num_atmcapvolstrikes
		, num_mktrate
		, num_indexmktrate
		, num_rngchkindexmktrate
		, num_swaptionoption
		, num_swaptionswap
		, num_mktcapvolstrikes
		, num_capkeytenor
		, num_terminalcorrtenor
		, raoptm_prd1
		, raoptm_prd2
		, raoptm_frq1
		, raoptm_frq2
	...
	vector<string> tenor(num_mktrate)
		, type(num_mktrate)
	...
	vector<string> indextenor(num_indexmktrate)
		, indextype(num_indexmktrate)
		, indexfixedrateleg_freq(num_indexmktrate)
		, indexfloatingrateleg_freq(num_indexmktrate);

	vector<double> indexmkt_rate(num_indexmktrate);
	
	for (i = 0; i < num_indexmktrate; i++)
	{
		fin >> indextenor[i];
		fin >> indextype[i];
		fin >> indexfixedrateleg_freq[i];
		fin >> indexfloatingrateleg_freq[i];
		fin >> indexmkt_rate[i]; 
	}

	num_rngchkindexmktrate = num_mktrate;
	
	vector<string> rngchkindextenor = tenor
		, rngchkindextype = type
		, rngchkindexfixedrateleg_freq = fixedrateleg_freq
		, rngchkindexfloatingrateleg_freq = floatingrateleg_freq;
	...
	vector<string> capkeytenor(num_capkeytenor)
		, capfreq(num_capkeytenor);
	...
	for (i = 0; i < num_mktcapvolstrikes; i++)
	{
		fin >> mktcapstrikes[i];
	}
	...
	for (i = 0; i < num_capkeytenor; i++)
	{
		...
		for (j = 0; j < num_mktcapvolstrikes; j++)
		{
			fin >> mktcapvol[i][j];
		}
	}
	...
	for (i = 0; i < num_terminalcorrtenor; i++)
	{
		...
		for (j = 0; j < num_terminalcorrtenor; j++)
		{
			fin >> label; fin >> terminalcorr[i][j]; 
		}
	}
	...
	int num_couponlegholidays = NumHolidays(couponleg_holiday)
		...
		, num_callhoidays = NumHolidays(call_holiday)
	...
	if (call_schedgen_flag)
	{
		...
		if (num_calldate > 0)
		{
			calculationstartschedule(callstart_date, callend_date, call_holidays, num_callhoidays, call_stub, call_direction, call_conv, call_freq, "ADJUSTED", num_calldate, calleffective_date);
		}
