...
)
{
    ...
    B_tT = (1-exp(-a*cvg(t_start, t_end, dcb)))/a;
...
)
{
    ...
    coeff 
        =   (
            (pow(sig, 2) / (4 * a))
            * (1 - exp(-2 * a * cvg(today, t_start, dcb)))
            );

    A_tT = (P_market_T / P_market_t)
        * exp(
            hw1f_B_tT(t_start, t_end, dcb, a) * fwd
            - coeff *pow(hw1f_B_tT(t_start, t_end, dcb, a),2)
        );
...
)
{
    ...
    Bond_P
        = hw1f_A_tT(P_market_T, P_market_t, fwd, today, t_start, t_end, dcb, a, sig)
        * exp(-hw1f_B_tT(t_start, t_end, dcb, a) * shortrate);
...
{
    double hw1f_sigma_p = 0.0;
    double coef_BtT = hw1f_B_tT(t_start, t_end, dcb, a);
    double coef_1 = sqrt((1 - exp(-2 * a * cvg(t_today, t_start, dcb))) / (2*a));
    double coef_2 = cvg(t_today, t_start, dcb);

    hw1f_sigma_p 
        = sig
        * sqrt((1-exp(-2 * a * cvg(t_today, t_start, dcb)))/(2*a))
        * hw1f_B_tT(t_start, t_end, dcb, a);
...
{
    double hw1f_h = 0.0;
    double coeff_1 = hw1f_sigma_p(today, t_start, t_end, dcb, a, sig);
    double coeff_2 = log(P_market_T * (1 + strike * cvg(t_start, t_end, dcb)) / P_market_t);
     double coeff_3 = cvg(t_start, t_end, dcb);

    hw1f_h
        = (1/ hw1f_sigma_p(today, t_start, t_end, dcb, a, sig))*log(P_market_T*(1+strike*cvg(t_start, t_end, dcb))/P_market_t)
        + 0.5 * hw1f_sigma_p(today, t_start, t_end, dcb, a, sig);

    return hw1f_h;
}

double hw1f_floorlet
(
    CDate today
    , CDate t_start
    , CDate t_end
...
{
    double hw1f_floorlet = 0.0;

    hw1f_floorlet
        = (1+strike*cvg(t_start, t_end, dcb))
        * hw1f_Bond_P(P_market_T, P_market_t, fwd, today, t_start, t_end, dcb, a, sig, shortrate)
        * cumnormal(hw1f_h(P_market_T, P_market_t, today, t_start, t_end, dcb, a, sig, strike))
        - hw1f_Bond_P(P_market_T, P_market_t, fwd, today, t_start, t_end, dcb, a, sig, shortrate)
        * cumnormal(hw1f_h(P_market_T, P_market_t, today, t_start, t_end, dcb, a, sig, strike) - hw1f_sigma_p(today, t_start, t_end, dcb, a, sig));

    return hw1f_floorlet;
}
...
// Created by Hokim 2020-03-27
void MyPlainSwapPrice_capfloor
(
    CCurrency crcy
    , CDate today
    , CDate settlement_date
...
)
{
    ...
    double h = 0.0;
    double sigma_p = 0.0;
    double effday = cvg(today, settlement_date, dcb);
    vector<double> coef_BtT(num_fundingleg_cf, 0.0);
    vector<double> sigmaP(num_fundingleg_cf, 0.0);
    vector<double> coef_h(num_fundingleg_cf, 0.0);
    vector<double> caplet(num_fundingleg_cf, 0.0);
    ...
    for (i = starti; i < num_fundingleg_cf; i++)
    {
        ...
        if (i == starti)
        {
            coef_BtT[i] = hw1f_B_tT(settlement_date, fundingleg_paydate[i], dcb, a[i]);
            sigmaP[i] = hw1f_sigma_p(today, settlement_date, fundingleg_paydate[i], dcb, a[i], sig[i]);
            coef_h[i] = hw1f_h(fundingleg_df[i], settlement_date_df, today, settlement_date, fundingleg_paydate[i], dcb, a[i], sig[i], strike);

            caplet[i]
                = settlement_date_df * cumnormal(-coef_h[i] + sigmaP[i])
                - (1 + strike * cvg(settlement_date, fundingleg_paydate[i], dcb))
                * fundingleg_df[i]
                * cumnormal(-coef_h[i]);
        }
        else
        {
            coef_BtT[i] = hw1f_B_tT(fundingleg_paydate[i - 1], fundingleg_paydate[i], dcb, a[i]);
            sigmaP[i] = hw1f_sigma_p(today, fundingleg_paydate[i - 1], fundingleg_paydate[i], dcb, a[i], sig[i]);
            coef_h[i] = hw1f_h(fundingleg_df[i], fundingleg_df[i - 1], today, fundingleg_paydate[i - 1], fundingleg_paydate[i], dcb, a[i], sig[i], strike);

            caplet[i]
                = fundingleg_df[i - 1] * cumnormal(-coef_h[i] + sigmaP[i])
                - (1 + strike * cvg(fundingleg_paydate[i - 1], fundingleg_paydate[i], dcb))
                * fundingleg_df[i]
                * cumnormal(-coef_h[i]);
        }
    ...
    , double vol)
{
    ...
    if (fwd > 0.0)
    {
        if (t > 0.0)
        {
            ans = df * tau * Bl(k, fwd, vol * sqrt(t), -1);
        }
        else if (t >= -tau)
        {
             ans = max(k - fwd, 0.0) * tau * df;
        }
    }
    else
    {
...
)
{
    ...
    for (i = startk; i < num_remained_couponleg_cf; i++)
    {
        fixedlegnonfixedcf[i] 
            = exp(-zc(_couponleg_paydate[i]) * (couponlegT[i] - callnotice_t))
            * _couponleg_notional[i]
            * couponlegtau[i]
            * _couponleg_couponrate[i];
