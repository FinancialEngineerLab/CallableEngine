...
)
{
	...
	else
	{
		...
		for (i=0; i<num_couponleg_cf; i++)
		{
			...
			couponleg_couponrate[i] = couponleg_couponrate[i] + couponleg_spread[i] + 0.05;
		}
	...
	if (num_calldate > 0)
	{
		...
		for (k = 0; k <= callendi - callstarti; k++)
		{
			...
			ShiftBusDate(
				callnotice_date[k+callstarti]
				, tempinst.get_holiday()
				, tempinst.get_numholiday()
				, 1
				, ondate);
		...
		for (k = callendi-callstarti; k >= 0; k--)
		{
			...
			for (l = Nt[k]-1; l >= 0; l--)
			{
				...
				for (ii = 1; ii < maxNx; ii++)
				{
					...
					//(2)
					A[ii] = tmpAt - (ii - Nx) * dx * a[k + callstarti] * 0.5;
					B[ii] = tmpBt - (ii - Nx) * dx;
					C[ii] = tmpAt + (ii - Nx) * dx * a[k + callstarti] * 0.5;
				}
